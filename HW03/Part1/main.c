...
int main (int argc, char **argv) {
  ...
  unsigned int n, p, g, h, x; //unsigned intigers
  ...
  //declare storage for an ElGamal cryptosytem move up top
  ...
  /* Q2.3 Have only Bob populate messages and then
    ...
    //make send rec decrypt
    MPI_Send(message, Nmessages, MPI_UNSIGNED, 0, 2, MPI_COMM_WORLD);//Nmessages instead of 1
  ...
  MPI_Recv(message, Nmessages, MPI_UNSIGNED, 1, 2, MPI_COMM_WORLD, status);//FINISH ThIS
